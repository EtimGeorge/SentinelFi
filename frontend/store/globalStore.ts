...
export const TIME_RANGE_PRESETS: Record<TimeRangeKey, { label: string; labelLong: string; days: number }> = {
  ...
  '30d': { label: 'Last 30 days', labelLong: 'Last 30 Days', days: 30 }, quarter: { label: 'This quarter', labelLong: 'This Quarter', days: 90 }, year: { label: 'This year', labelLong: 'This Year', days: 365 },
};
...
const useGlobalStore = create<GlobalState>()(
  persist(
    (set, get) => ({
      selectedProjectId: 'all', selectedCurrencyCode: 'USD', timeRange: '30d', savedViews: [], setSelectedProjectId: (projectId: string) => set({ selectedProjectId: projectId }), setSelectedCurrencyCode: (currencyCode: string) => set({ selectedCurrencyCode: currencyCode }), setTimeRange: (range: TimeRangeKey) => set({ timeRange: range }), saveView: (name: string) => {
    ...
    {
      name: 'sentinelfi-global-context', storage: createJSONStorage(() => sessionStorage),
    }
  )
);
