            ...
            <div className="p-6 bg-slate-900/40 backdrop-blur-xl border border-slate-800 rounded-3xl elev-lg sticky top-24">
              ...
              <div className="space-y-6">
                ...
                <div className="p-1 bg-slate-950 rounded-2xl flex border border-slate-800">
                  <button onClick={() => setViewMode('analytics')} className={`flex-1 py-1.5 rounded-xl text-xs font-black  transition-all ${viewMode === 'analytics' ? 'bg-brand-primary text-white elev-lg' : 'text-slate-500 hover:text-slate-300'}`}>Analytics</button>
                  <button onClick={() => setViewMode('report')} className={`flex-1 py-1.5 rounded-xl text-xs font-black  transition-all ${viewMode === 'report' ? 'bg-brand-primary text-white elev-lg' : 'text-slate-500 hover:text-slate-300'}`}>Report</button>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(['daily', 'weekly', 'monthly', 'all'] as const).map((opt) => (
                      <button key={opt} onClick={() => setInterval(opt)} className={`px-3 py-2 rounded-xl border text-xs font-black uppercase tracking-tighter transition-all ${interval === opt ? 'bg-brand-primary border-brand-primary text-white elev-lg' : 'bg-slate-900/50 border-slate-800 text-slate-500 hover:border-slate-700'}`}>{opt}</button>
                    ))}
                  </div>
                </div>
                ...
                {summary && (
                  <div className="space-y-3 pt-4 border-t border-slate-800">
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-black text-slate-500 uppercase">Efficiency</span>
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-black text-slate-500 uppercase">Total Budget</span>
                      <span className="text-sm font-bold text-white">{convertToDisplay(summary.totalBudgeted, 'NGN', false)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-black text-slate-500 uppercase">Actual Burn</span>
                ...
                <div className="pt-4 border-t border-slate-800 space-y-3">
                  ...
                  <button onClick={handlePushToDCS} disabled={loading} className="w-full flex items-center gap-3 px-4 py-3 bg-alert-warning/10 hover:bg-alert-warning/20 border border-alert-warning/20 rounded-xl text-alert-warning text-xs font-black  transition-all disabled:opacity-30">
            ...
            {viewMode === 'analytics' && summary && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in slide-in-from-top-4 duration-500">
                <Card className="bg-slate-900/80 border-slate-800 p-6 flex items-center gap-5">
                  ...
                  <div>
                    <p className="text-xs font-black text-brand-primary ">OPEX Efficiency</p>
                ...
                <Card className="bg-slate-900/80 border-slate-800 p-6 flex items-center gap-5">
                  ...
                  <div>
                    <p className="text-xs font-black text-red-400 ">Total Variance</p>
                ...
                <Card className="bg-slate-900/80 border-slate-800 p-6 flex items-center gap-5">
                  ...
                  <div>
                    <p className="text-xs font-black text-emerald-500 ">Active Budgets</p>
              ...
              <div className="max-w-4xl mx-auto space-y-10 py-8">
                ...
                {!loading && rollup?.budgets && rollup.budgets.length > 0 ? (
                  <div className="space-y-8">
                    <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
                      ...
                      <h4 className="text-xs font-black text-slate-900 ">Budget → Category Drill-Down</h4>
                    </div>
                          ...
                          >
                            <div className="flex items-center gap-4">
                              ...
                              <div className="text-left">
                                ...
                                <p className="text-xs font-bold text-slate-400 ">{budget.type} · {budget.status}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-8 text-right">
                              <div>
                                <p className="text-xs font-black text-slate-400 uppercase">Budget</p>
                                <p className="font-black text-slate-900">{convertToDisplay(budget.budgeted, 'NGN', false)}</p>
                              </div>
                              <div>
                                <p className="text-xs font-black text-slate-400 uppercase">Actual</p>
                                <p className="font-black text-slate-700">{convertToDisplay(budget.actual, 'NGN', false)}</p>
                              </div>
                              <div>
                                <p className="text-xs font-black text-slate-400 uppercase">Burn</p>
                          ...
                          {(isExpanded || viewMode === 'report') && budget.categories.length > 0 && (
                            <div className="border-t border-slate-200/50">
                              <table className="w-full text-sm">
                                <thead>
                                  <tr className="text-xs font-black text-slate-400  border-b border-slate-200/70 bg-white/70">
                                    ...
                                    <tr key={cat.id} className="hover:bg-white/50 transition-colors">
                                      <td className="pl-16 pr-4 py-3.5">
                                        <div className="flex items-center gap-2">
                                          ...
                                          <span className={`ml-1 px-1.5 py-0.5 rounded-md text-xs font-black uppercase ${cat.status === 'OVERRUN' ? 'bg-red-100 text-red-600' : cat.status === 'AT_RISK' ? 'bg-amber-100 text-amber-600' : 'bg-emerald-100 text-emerald-700'}`}>{cat.status}</span>
                                        </div>
                                      </td>
                ...
                <footer className="pt-12 border-t border-slate-200 mt-20 flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-black text-slate-400 ">Enterprise Compliance Verified</span>
                  </div>
                  <div className="text-right">
                    <p className="text-xs font-black text-slate-900 ">{userCurrency.code} Valuation</p>
                    <p className="text-xs text-slate-400 font-medium">SentinelFi OPEX Hub</p>
                  </div>
                </footer>
              </div>
            </div>
