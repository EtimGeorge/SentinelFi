...
const ProjectBudgetPreviewPage: React.FC = () => {
  ...
  // Extend base type to include API rollup properties
  ...
  const handleInlineEdit = (id: string) => {
    router.push(`/wbs-manager?edit=${id}`);
  };
  ...
  const renderTree = (parentId: string | null = null, level: number = 0) => {
    ...
    return children.map((item: WbsBudgetExtended) => {
  ...
  const renderFlatTable = () => {
    return budgets.map((item: WbsBudgetExtended) => {
      const spent = Number(item.total_paid_rollup || 0);
      ...
      <PageContainer
        ...
        headerContent={
          <div className="flex items-center gap-4 print:hidden">
            ...
            <Link href="/budget" className="flex items-center text-sm font-bold text-gray-400 hover:text-white transition">
