...
const TenantProjectSetupPage: React.FC = () => {
  ...
  // Auto-save setup
  const { autoSave, restoreData, clearData } = useFormAutoSave('tenant-setup-form');

  // Restore draft on mount
  useEffect(() => {
    const restored = restoreData<any>();
    if (restored && (restored.name || restored.projectName)) {
      if (confirm('Restore previous tenant setup draft?')) {
        setFormData(prev => ({ ...prev, name: restored.name, projectName: restored.projectName }));
      }
    }
  }, []);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData(prev => {
      ...
      // Auto-save only text fields, exclude file
      autoSave({ name: updated.name, projectName: updated.projectName });
      return updated;
    });
  };
