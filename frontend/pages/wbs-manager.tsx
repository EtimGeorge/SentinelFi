...
import { TrendingUp, Plus, Trash2, Edit3, Save, X, AlertTriangle, ChevronRight, ChevronDown, Layers, MessageSquare } from 'lucide-react';
...
const WBSManagerPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ code: '', description: '', amount: 0, projectId: '' });

  // Auto-save setup
  const { autoSave, restoreData, clearData } = useFormAutoSave('wbs-item-form');

  // Restore draft when opening existing action node
  useEffect(() => {
    // Only restore if ActionNode has just been set to 'add' (fresh form)
    if (actionNode && actionNode.type === 'add') {
      const restored = restoreData<any>();
      // Only prompt if restored data is different from default/calculated
  ...
  }, [actionNode]); // Depend on actionNode opening
  ...
  const fetchWBSData = useCallback(async () => {
    setLoading(true);
    try {
      // Use the rollup endpoint with project filter
  ...
  }, [api, addToast, selectedProjectId]);
  ...
  const handleAction = (type: 'add' | 'edit', parentId: string | null = null, node?: WBSItem) => {
    ...
    setFormData({
      ...
      projectId: (node as any)?.project_id || (parentId ? (items.find(i => i.wbs_id === parentId) as any)?.project_id : (selectedProjectId !== 'all' ? selectedProjectId : ''))
    });
  };
  ...
  const saveAction = async () => {
    ...
    try {
      ...
      clearData(); // Clear draft on success
      setActionNode(null);
      fetchWBSData();
    } catch (e: any) {
            ...
            <div className="flex items-center flex-grow min-w-0">
              ...
              <div className="flex flex-col truncate">
                <div className="flex items-center gap-2">
                  ...
                  {(item as any).has_annotations && (
                    <MessageSquare className="w-3 h-3 text-brand-primary animate-pulse" />
                  )}
                </div>
                <span className="text-gray-200 text-sm truncate pr-4 mt-0.5">{item.description}</span>
              </div>
            </div>

            <div className="text-right mr-6">
              <p className="font-black text-sm text-gray-100">{formatCurrency(item.total_cost_budgeted)}</p>
              <p className="text-[10px] text-gray-500 uppercase font-bold tracking-tighter">Allocated</p>
            </div>

            {canManage && (
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => handleAction('add', item.wbs_id)} className="p-2 text-brand-primary hover:bg-brand-primary/20 rounded-lg transition" title="Add Child">
                  <Plus className="w-4 h-4" />
                </button>
                <button onClick={() => handleAction('edit', item.parent_wbs_id, item)} className="p-2 text-gray-400 hover:bg-gray-700 rounded-lg transition" title="Edit">
                  <Edit3 className="w-4 h-4" />
                </button>
                <button onClick={() => handleDelete(item.wbs_id)} className="p-2 text-red-500 hover:bg-red-900/40 rounded-lg transition" title="Delete">
            ...
            {actionNode ? (
              ...
              >
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Budgeted Amount ({displayCurrency})</label>
                    <input
                      type="number"
                      value={formData.amount}
                      onChange={e => {
                        const updated = { ...formData, amount: parseFloat(e.target.value) || 0 };
                        setFormData(updated);
                        autoSave(updated);
                      }}
