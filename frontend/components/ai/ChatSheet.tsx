...
import { Bot, X, Send, Paperclip, Sparkles, Loader, Copy, CheckCircle, ChevronRight, TrendingUp, TrendingDown, DollarSign, Table, Grid, Trash2, AlertTriangle, Zap } from 'lucide-react';
import { AIChatMessage, RichContent } from './types';
import { useAIChat } from '../../hooks/useAIChat';
import DataTable from "../../components/common/DataTable";

const SHEET_STYLES = `
  @keyframes sentinelai-sheet-enter {
  ...
  .sentinelai-sheet { animation: sentinelai-sheet-enter 0.4s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
...
const RichContentRenderer: React.FC<{ content: RichContent }> = ({ content }) => {
  switch (content.type) {
    case 'text':
      return <div className="text-white/90 whitespace-pre-wrap">{content.value}</div>;
    case 'chart':
      return (
        <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4 mt-2">
          <div className="flex items-center justify-between mb-3">
            <h4 className="font-bold text-white">{content.config.title || 'Chart'}</h4>
            <span className="text-xs text-gray-500 capitalize">{content.config.type}</span>
          </div>
          ...
          {content.chips.map((chip, i) => (
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                chip.variant === 'primary' ? 'bg-brand-primary text-white' :
                chip.variant === 'destructive' ? 'bg-red-600/20 text-red-400 border border-red-600/30' :
    ...
    case 'kpi':
      return (
        <div className={`bg-gray-800/50 border border-gray-700 rounded-xl p-4 mt-2 border-l-4`} style={{ borderLeftColor: content.accent }}>
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-gray-500 uppercase">{content.label}</span>
            {content.trend !== undefined && (
              <span className={`text-xs font-bold ${content.trend >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                {content.trend >= 0 ? <TrendingUp className="w-3 h-3 inline" /> : <TrendingDown className="w-3 h-3 inline" />}
  ...
  if (!isAssistant) {
    return (
      <div className="sentinelai-msg-appear flex justify-end gap-3">
        <div className="max-w-[85%] px-4 py-3 bg-gradient-to-br from-brand-primary/30 to-brand-secondary/20 border border-brand-primary/30 rounded-2xl rounded-br-sm text-white">
          {typeof message.content === 'string' ? message.content : <RichContentRenderer content={message.content as RichContent} />}
        </div>
...
export const ChatSheet: React.FC<ChatSheetProps> = ({
  ...
  const { sendMessage: hookSendMessage } = useAIChat();
  ...
  return (
    <>
      ...
      {/* Sheet */}
      <div
        className="sentinelai-sheet fixed bottom-0 left-0 right-0 z-[9999] md:hidden bg-brand-dark/95 backdrop-blur-3xl border-t border-white/5 rounded-t-3xl flex flex-col max-h-[90vh] overflow-hidden"
        role="dialog"
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-700 bg-black/20">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-bold text-white text-lg">SentinelFi AI</span>
                <span className={`w-2 h-2 rounded-full ${isStreaming ? 'bg-yellow-400' : 'bg-green-400'}`} />
              </div>
              <div className="text-xs text-gray-500">Analyzing: {scope?.page || 'Dashboard'}</div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {messages.length > 0 && (
        ...
        {/* Proactive Insight */}
        {proactiveInsight && (
          <div className="px-4 py-3 bg-yellow-900/20 border-b border-yellow-700/30">
            <div className="flex items-start gap-3">
              <AlertTriangle className="w-5 h-5 text-yellow-400 flex-shrink-0 mt-0.5" />
              <div className="flex-1">
                <p className="font-bold text-yellow-300">{proactiveInsight.title}</p>
                <p className="text-sm text-yellow-200 mt-1">{proactiveInsight.message}</p>
                {proactiveInsight.actionPrompt && (
                  <button
                    onClick={() => onQuickAction(proactiveInsight.actionPrompt)}
        ...
        {(messages.length === 0 || showQuickActions) && (
          <div className="px-4 py-3 border-b border-gray-700/50">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-bold text-gray-500 r">Quick Actions</p>
              <button
            ...
            <div className="flex flex-wrap gap-2">
              {quickActions.slice(0, 6).map((action, i) => (
                <button
                  key={i}
                  onClick={() => { onQuickAction(action.prompt); setShowQuickActions(false); }}
                  className="px-3 py-2 rounded-xl text-sm font-medium border border-brand-primary/30 text-brand-primary hover:bg-brand-primary/10 transition-colors flex items-center gap-2"
                >
        ...
        >
          ...
          {messages.length === 0 && (
            <div className="text-center py-16 text-gray-500">
              <Sparkles className="w-16 h-16 mx-auto text-gray-700 mb-4" />
              <p className="font-medium text-white text-lg">Start a conversation</p>
              <p className="text-xs mt-1">Ask me about budgets, projects, or expenses</p>
            </div>
          )}
        </div>

        {/* Input Area */}
        <div className="p-4 border-t border-gray-700/50 bg-black/20 pb-safe">
          <div className="relative">
            <div className="flex items-end gap-2 bg-gray-900/50 border border-gray-700 rounded-2xl p-2 transition-colors focus-within:border-brand-primary/50">
              ...
              <textarea
                ...
                className="flex-1 bg-transparent border-none text-white text-base resize-none max-h-40 outline-none placeholder:text-gray-500"
              ...
              <button
                ...
                className="p-3 rounded-xl transition-colors flex-shrink-0 ${inputValue.trim() && !isStreaming
                  ? 'bg-brand-primary text-white hover:bg-brand-primary/80'
                  : 'bg-gray-800 text-gray-500 cursor-not-allowed'}"
                aria-label="Send message"
              >
                {isStreaming ? <Loader className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5" />}
              </button>
            </div>
