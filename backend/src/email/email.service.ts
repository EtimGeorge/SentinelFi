...
export class EmailService implements OnModuleInit {
  ...
  private initializeProvider() {
    ...
    this.provider = providerType === 'smtp'
  ...
  async sendEmailWithAttachment(
    ...
    if (
      ...
      this.logger.warn(`[EMAIL PREVIEW+ATTACH] To: ${to} | Subject: ${subject} | Files: ${attachments?.length ?? 0}`);
      return;
    }
    try {
      if (typeof (this.provider as any).sendEmailWithAttachment === 'function') {
        await (this.provider as any).sendEmailWithAttachment({ to, subject, html, attachments });
      } else {
        this.logger.warn(`Provider '${this.provider.getName()}' does not support attachments. Sending without PDF.`);
        await this.provider.sendEmail({ to, subject, html });
      }
    } catch (error) {
      this.logger.error(`Failed to send email with attachment via ${this.provider.getName()}:`, error);
      throw error;
    }
  }
  ...
  private renderTemplate(templateName: string, context: Record<string, any>): string {
    ...
    if (!fs.existsSync(templatePath)) throw new Error(`Email template not found: ${templateName}`);

    const source = fs.readFileSync(templatePath, 'utf8');
    const bodyHtml = Handlebars.compile(source)(context);
    const appUrl = this.configService.get<string>('FRONTEND_URL', 'https://sentinelfi.com');

    return this.layoutTemplate
      ? this.layoutTemplate({ body: bodyHtml, year: new Date().getFullYear(), appUrl, ...context })
      : bodyHtml;
  }
  ...
  async sendPaymentReceiptEmail(
    ...
    const html = this.renderTemplate('payment-receipt', { ...context, hasAttachment: !!pdfBuffer });
    if (pdfBuffer) {
      await this.sendEmailWithAttachment(to, subject, html, [
        { filename: `SentinelFi-Invoice-${context.invoiceNumber}.pdf`, content: pdfBuffer, contentType: 'application/pdf' },
      ]);
    } else {
      await this.sendEmail(to, subject, html);
    }
    this.logger.log(`[EmailService] Receipt → ${to} (${context.invoiceNumber})`);
  }

  async sendTrialActivationEmail(to: string, context: TrialActivationContext): Promise<void> {
  ...
  async sendSubscriptionSuccessEmail(to: string, context: SubscriptionSuccessContext): Promise<void> {
  ...
  async sendPaymentFailureEmail(to: string, context: PaymentFailedContext): Promise<void> {
  ...
  async sendRenewalReminderEmail(to: string, context: RenewalReminderContext): Promise<void> {
    ...
    this.logger.log(`[EmailService] Renewal reminder (${context.daysRemaining}d) → ${to}`);
  }
  ...
  async sendSubscriptionRenewedEmail(
    ...
    const html = this.renderTemplate('subscription-renewed', { ...context, hasAttachment: !!pdfBuffer });
    if (pdfBuffer) {
      await this.sendEmailWithAttachment(to, subject, html, [
        { filename: `SentinelFi-Renewal-${context.invoiceNumber}.pdf`, content: pdfBuffer, contentType: 'application/pdf' },
      ]);
    } else {
      await this.sendEmail(to, subject, html);
    }
  ...
  async sendTrialExpiryWarningEmail(to: string, context: TrialExpiryWarningContext): Promise<void> {
