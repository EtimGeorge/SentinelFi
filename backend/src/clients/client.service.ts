...
export class ClientService {
  ...
  async create(createClientDto: CreateClientDto): Promise<ClientEntity> {
    ...
    if (existingClient) {
      this.logger.warn(
        `Attempt to create duplicate client name: "${createClientDto.name}" for tenant: ${tenantId}`,
      );
    ...
    try {
      ...
      this.logger.log(
        `Client created successfully: ${savedClient.id} - "${savedClient.name}" for tenant: ${tenantId}`,
      );
  ...
  async findOne(id: string): Promise<ClientEntity> {
    ...
    if (!client) {
      throw new NotFoundException(
        `Client with ID "${id}" not found or has been deleted`,
      );
    }
    ...
    const portfolioMetrics = projects.map((p) => {
      ...
      const totalActual = p.wbsBudgets.reduce(
        (acc: number, w) => acc + Number(w.total_cost_actual),
        0,
      );
  ...
  async remove(id: string): Promise<void> {
    ...
    this.logger.log(
      `Client soft-deleted: ${id} - "${client.name}" for tenant: ${tenantId}`,
    );
  }
}
