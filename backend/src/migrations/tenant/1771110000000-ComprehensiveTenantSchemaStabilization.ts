...
export class ComprehensiveTenantSchemaStabilization1771110000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
            DO $$ BEGIN
                IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'budget_category_type_enum' AND typnamespace = current_schema()::regnamespace) THEN
                    CREATE TYPE "budget_category_type_enum" AS ENUM('CAPEX', 'OPEX');
                END IF;
                IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'period_type_enum' AND typnamespace = current_schema()::regnamespace) THEN
    ...
    await queryRunner.query(`
            DO $$ BEGIN
                -- clients.deleted_at
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'clients' AND column_name = 'deleted_at') THEN AND table_schema = current_schema()
                    ALTER TABLE "clients" ADD COLUMN "deleted_at" TIMESTAMP WITH TIME ZONE;
                END IF;
                
                -- live_expense.vat_amount / wht_amount
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'live_expense' AND column_name = 'vat_amount') THEN AND table_schema = current_schema()
                    ALTER TABLE "live_expense" ADD COLUMN "vat_amount" numeric(19,4) NOT NULL DEFAULT '0';
                END IF;
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'live_expense' AND column_name = 'wht_amount') THEN AND table_schema = current_schema()
    ...
    await queryRunner.query(`
            DO $$ BEGIN
                ...
                IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'FK_payroll_entry_budget' AND connamespace = current_schema()::regnamespace) THEN
                ...
                IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'FK_payroll_entry_processed_by' AND connamespace = current_schema()::regnamespace) THEN
                ...
                END IF;
                -- allocation -> category
                IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'FK_period_allocation_category' AND connamespace = current_schema()::regnamespace) THEN
