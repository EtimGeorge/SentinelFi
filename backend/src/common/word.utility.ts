        ...
        return new TableRow({
          children: [
            ...
            new TableCell({
              children: [
                new Paragraph(
                  WordUtility.formatCurrencyWithContext(
                    project.total_budgeted_rollup,
                    context,
                  ),
                ),
      ...
      sections: [
        {
          ...
          children: [
            new Paragraph({
              children: [
                new TextRun({
                  text: context?.tenantName || "SentinelFi",
                  bold: true,
                  color: "4A5568",
                  size: 20,
                }),
      ...
      ...expenses.map((expense) => {
        return new TableRow({
          children: [
            ...
            new TableCell({ children: [new Paragraph(expense.description || "No Description")] }),
  ...
  static async generateOpexRollupReport(
    ...
    const kpiRows = [
      ...
      ...(
        [
          ...
          [
            "Total Actual Burn",
            WordUtility.formatCurrencyWithContext(s.totalActual ?? 0, context),
          ],
          [
            "Net Variance",
            `${(s.totalVariance ?? 0) >= 0 ? "+" : ""}${WordUtility.formatCurrencyWithContext(s.totalVariance ?? 0, context)}`,
          ],
        ...
        ([k, v]) =>
          new TableRow({
            children: [
              ...
              new TableCell({
                children: [
                  new Paragraph({ children: [new TextRun({ text: v })] }),
                ],
              }),
            ],
          }),
      ...
      children: [
        ...
        (h) =>
          new TableCell({
            ...
            shading: { fill: "1A202C" },
            width: {
              size: h === "Budget / Category" ? 3200 : 1200,
              type: WidthType.DXA,
            },
          }),
      ),
    ...
    for (const budget of data.budgets ?? []) {
      ...
      breakdownRows.push(
        new TableRow({
          children: [
            ...
            makeCell(
              WordUtility.formatCurrencyWithContext(budget.budgeted, context),
              { bold: true, shade: true },
            ),
            makeCell(
              WordUtility.formatCurrencyWithContext(budget.actual, context),
              { bold: true, shade: true },
            ),
            makeCell(
              `${(budget.variance ?? 0) >= 0 ? "+" : ""}${WordUtility.formatCurrencyWithContext(budget.variance, context)}`,
              { bold: true, shade: true },
            ),
            makeCell(`${(budget.burnRate ?? 0).toFixed(1)}%`, {
              bold: true,
              shade: true,
              color: healthColor,
            }),
      ...
      for (const cat of budget.categories ?? []) {
        const catHealth =
          cat.burnRate > 100 ? "OVERRUN" : cat.burnRate > 85 ? "AT RISK" : "OK";
        const catColor =
          cat.burnRate > 100
            ? "DC2626"
            : cat.burnRate > 85
        ...
        breakdownRows.push(
          new TableRow({
            children: [
              ...
              makeCell(
                WordUtility.formatCurrencyWithContext(cat.budgeted, context),
              ),
              makeCell(
                WordUtility.formatCurrencyWithContext(cat.actual, context),
              ),
              makeCell(
                `${(cat.variance ?? 0) >= 0 ? "+" : ""}${WordUtility.formatCurrencyWithContext(cat.variance, context)}`,
              ),
              makeCell(`${(cat.burnRate ?? 0).toFixed(1)}%`, {
                color: catColor,
              }),
