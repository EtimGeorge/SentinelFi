...
class ConnectionCircuitBreaker {
  ...
  recordFailure(): void {
    ...
    if (this.failureCount >= this.failureThreshold) {
      if (this.state !== CircuitState.OPEN) {
        this.logger.error(
          `⚠️ Circuit breaker OPEN after ${this.failureCount} failures`,
        );
...
export class DatabaseConfig {
  ...
  static getTypeOrmConfig(
    ...
    this.logger.log(
      `Using sanitized DATABASE_URL (len: ${databaseUrl.length}): ${redactedUrl}`,
    );
  ...
  static async validateConnection(dataSource: DataSource): Promise<boolean> {
    if (!this.circuitBreaker.canAttemptConnection()) {
      this.logger.warn(
        "Circuit breaker is OPEN, rejecting connection validation",
      );
      return false;
    }
  ...
  private static async attemptReconnection(
    ...
    try {
      ...
      const backoffDelay = Math.min(
        this.RETRY_DELAY * Math.pow(2, this.connectionAttempts),
        30000,
      );
      this.logger.log(
        `Waiting ${backoffDelay}ms before reconnection attempt ${this.connectionAttempts + 1}/${this.MAX_RETRY_ATTEMPTS}`,
      );
      ...
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error("Pool initialization timed out")),
            initTimeout,
          ),
        ),
      ]);
      ...
      this.circuitBreaker.recordFailure();
      this.logger.error(
        `Reconnection attempt ${this.connectionAttempts} failed: ${errorMessage}`,
      );
      ...
      if (this.connectionAttempts < this.MAX_RETRY_ATTEMPTS) {
        ...
        this.logger.error(
          `⚠️ Max reconnection attempts (${this.MAX_RETRY_ATTEMPTS}) reached. Manual intervention required.`,
        );
      }
    } finally {
  ...
  private static setupPoolEventHandlers(dataSource: DataSource): void {
    try {
      ...
      if (!pool) {
        this.logger.warn(
          "Connection pool not accessible, cannot setup event handlers",
        );
        return;
      }
      ...
      pool.on("error", async (err: Error) => {
        ...
        if (isTerminalError) {
          this.logger.error(
            `🔴 Terminal connection error detected, initiating reconnection: ${msg}`,
          );
  ...
  private static startKeepAliveScheduler(dataSource: DataSource): void {
    this.logger.log(
      `Starting keep-alive scheduler (interval: ${this.KEEP_ALIVE_INTERVAL / 1000}s)`,
    );
    ...
    this.keepAliveInterval = setInterval(async () => {
      try {
        ...
        this.logger.debug(
          `Keep-alive query executed successfully (${duration}ms)`,
        );
        this.circuitBreaker.recordSuccess();
      } catch (error) {
        const errorMessage =
        ...
        if (isTerminal) {
          this.logger.warn(
            `Triggering emergency reconnection from keep-alive failure: ${errorMessage}`,
          );
          await this.attemptReconnection(dataSource);
        }
      }
    ...
    this.healthCheckInterval = setInterval(async () => {
      try {
        ...
        if (!dataSource.isInitialized && !this.isReconnecting) {
          this.logger.error(
            "🔴 DataSource is NOT initialized! Attempting recovery...",
          );
          await this.attemptReconnection(dataSource);
          return;
        }
        ...
        if (pool) {
          ...
          this.logger.log(
            `Pool Health - Total: ${totalCount}, Idle: ${idleCount}, Waiting: ${waitingCount} | Valid: ${isValid} | Circuit: ${circuitState}`,
          );
          ...
          if (!isValid) {
            ...
            if (consecutiveFailures >= 2) {
              this.logger.error(
                `🔴 Connection invalid for ${consecutiveFailures} consecutive checks. Forcing reconnection.`,
              );
              await this.attemptReconnection(dataSource);
          ...
          if (waitingCount > 5) {
            this.logger.warn(
              `⚠️ Pool Exhaustion Warning: ${waitingCount} queries waiting for connections!`,
            );
          }
          ...
          if (totalCount === 0 && !this.isReconnecting) {
            this.logger.error(
              "🔴 Pool is empty! Initiating emergency reconnection...",
            );
            await this.attemptReconnection(dataSource);
          }
        } else if (!this.isReconnecting) {
          this.logger.error(
            "🔴 Postgres pool object missing in health check. Initiating reconnection.",
          );
          await this.attemptReconnection(dataSource);
        }
      } catch (error) {
  ...
  static async shutdown(dataSource: DataSource): Promise<void> {
    ...
    try {
      if (dataSource.isInitialized) {
        // Check if the DataSource is initialized before destroying
        await dataSource.destroy();
        this.logger.log("✓ Database connections closed gracefully");
      } else {
        this.logger.warn(
          "Database DataSource was not initialized or already destroyed. Skipping destroy operation.",
        );
      }
    } catch (error) {
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Unknown error during database shutdown";
...
export class SafeTransaction {
  ...
  static async execute<T>(
    ...
    this.logger.log(
      `[SafeTransaction] Starting transaction. Timeout: ${timeoutMs}ms. CID: ${correlationId}`,
    );
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this.logger.log(
        `[SafeTransaction] Transaction COMPLETED in ${duration}ms (CID: ${correlationId})`,
      );
      return result;
    } catch (error) {
      const duration = Date.now() - startTime;
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error";
      this.logger.error(
        `[SafeTransaction] Transaction FAILED after ${duration}ms: ${errorMessage} (CID: ${correlationId})`,
      );
      throw error;
    } finally {
  ...
  static async execute<T>(
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (
          (lastError as DbError).code === "23505" ||
          (lastError as DbError).code === "23503"
        ) {
          throw lastError;
        }

        if (attempt < maxRetries - 1) {
          const delay = lastError.message.includes("EAI_AGAIN")
            ? 500
            : baseDelay * Math.pow(2, attempt); // Faster retry for DNS
          this.logger.warn(
            `Query failed (attempt ${attempt + 1}/${maxRetries}), retrying in ${delay}ms... Error: ${lastError.message}`,
          );
    ...
    if (lastError) {
      // Check if lastError was assigned
      throw lastError;
    } else {
