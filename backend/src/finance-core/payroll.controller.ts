...
export class PayrollController {
  ...
  @Post('runs')
  @Roles(Role.CFO, Role.FinanceManager, Role.AdminDirector, Role.AdminManager, Role.SuperAdmin, Role.CEO, Role.TechnicalDirector)
  createRun(@Body() data: { runIdentifier: string; fiscalPeriodId: string; runDate: string }) {
  ...
  @Post('runs/:id/items')
  @Roles(Role.CFO, Role.FinanceManager, Role.AdminDirector, Role.AdminManager, Role.SuperAdmin, Role.CEO, Role.TechnicalDirector)
  addLineItem(@Param('id') id: string, @Body() data: {
    employeeId: string;
    costCenterId: string;
    glAccountId: string;
