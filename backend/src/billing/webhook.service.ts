...
export class WebhookService {
  ...
  private async isNewEvent(eventId: string, provider: string, payload: any): Promise<boolean> {
    const existing = await this.processedWebhookRepository.findOne({
      where: { gateway_event_id: eventId },
    });

    if (existing) {
      this.logger.warn(`Duplicate webhook received from ${provider}: ${eventId}. Skipping.`);
      return false;
    }
