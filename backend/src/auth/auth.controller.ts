...
export class AuthController {
  ...
  async loginSuperAdmin(
    ...
    try {
      ...
      this.logger.error(
        `SuperAdmin login failed: ${error instanceof Error ? error.message : String(error)}`,
  ...
  async loginTenant(
    ...
    try {
      ...
      this.logger.error(
        `Tenant login failed: ${error instanceof Error ? error.message : String(error)}`,
        error instanceof Error ? error.stack : undefined,
      );
      if (error instanceof HttpException) {
        ResponseHelper.sendError(res, error.getStatus(), error.message);
  ...
  async logout(
    ...
    const tokenPayload = (req as any).user as
  ...
  async getUsers(@Req() req: AuthenticatedRequest): Promise<UserResponseDto[]> {
    ...
    return this.authService.findAllUsers(
      isSuperAdmin ? undefined : (req.user.tenant_id ?? undefined),
    );
  }
  ...
  async batchUpdateUsers(
    ...
    if (
      !batchDto.ids ||
      !Array.isArray(batchDto.ids) ||
      batchDto.ids.length === 0
    ) {
      throw new BadRequestException("Multiple User IDs must be provided.");
    }
    return this.authService.batchUpdateUsers(
      req.user,
      batchDto.ids,
      batchDto.update,
    );
  ...
  async getUserProfile(
    ...
    return this.authService.findUserProfileDossier(
      id,
  ...
  async impersonate(
    ...
    try {
      const { access_token, user } = await this.authService.impersonate(
        impersonator,
        targetUserId,
      );
      ...
      this.logger.error(
        `Impersonation failed: ${error instanceof Error ? error.message : String(error)}`,
        error instanceof Error ? error.stack : undefined,
      );
      if (error instanceof HttpException) {
        ResponseHelper.sendError(res, error.getStatus(), error.message);
        ...
        ResponseHelper.sendError(
          res,
          HttpStatus.INTERNAL_SERVER_ERROR,
          "An unexpected internal error occurred during impersonation.",
        );
      }
    }
  }
  ...
  async stopImpersonation(
    ...
    try {
      ...
      ResponseHelper.sendJson(res, HttpStatus.OK, {
        success: true,
        user,
        message: "Impersonation stopped. Restored SuperAdmin session.",
      });
    } catch (error) {
      this.logger.error(
        `Stop impersonation failed: ${error instanceof Error ? error.message : String(error)}`,
        error instanceof Error ? error.stack : undefined,
      );
      if (error instanceof HttpException) {
        ResponseHelper.sendError(res, error.getStatus(), error.message);
        ...
        ResponseHelper.sendError(
          res,
          HttpStatus.INTERNAL_SERVER_ERROR,
          "An unexpected internal error occurred while stopping impersonation.",
        );
      }
    }
  }
