...
export class PermissionsGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!user || !user.permissions || user.permissions.length === 0) {
      throw new ForbiddenException('Insufficient permissions: User has no assigned permissions.');
    }
    ...
    if (!hasAllRequiredPermissions) {
      throw new ForbiddenException('Insufficient permissions: Missing one or more required permissions.');
    }
