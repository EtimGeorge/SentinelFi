...
export class SuperAdminService {
  ...
  async impersonateUser(userId: string, superAdminId: string): Promise<string> {
    ...
    this.auditService
      ...
      .catch((err) =>
        this.logger.error(`Failed to log impersonation start: ${err.message}`),
      );
  ...
  async getTenantDetails(tenantId: string): Promise<TenantDetailDto> {
    ...
    const adminUsers = await this.userRepository
      ...
      .andWhere("role.name IN (:...roleNames)", {
        roleNames: [Role.AdminDirector, Role.CEO],
      })
    ...
    try {
      const lastActivity = await this.dataSource
      ...
      try {
        ...
        this.logger.warn(
          `Could not fetch wbs_budget count for tenant ${tenant.schema_name}`,
        );
      }
      ...
      try {
        ...
        this.logger.warn(
          `Could not fetch live_expense count for tenant ${tenant.schema_name}`,
        );
      }

      const recentAuditLogs = await this.dataSource
        .getRepository(AuditLogEntity)
        .find({
          where: { tenantId: tenantId },
          take: 5,
  ...
  private fillMissingDates(
    ...
    const dataMap = new Map(
      data.map((item) => [item.date, parseInt(item.count)]),
    );
  ...
  async getWbsMetrics(tenantId?: string): Promise<any> {
    ...
    const metricResults = await Promise.all(
      tenants.map(async (tenant) => {
        try {
          ...
          this.logger.warn(
            `Failed to fetch WBS metrics for tenant ${tenant.name} (${tenant.schema_name})`,
          );
    ...
    const totalBudget = metricResults.reduce(
  ...
  async getOperationalBudgetMetrics(tenantId?: string): Promise<any> {
    ...
    return {
      ...
      averageBudgetUtilization:
        totalAllocated > 0 ? totalUsed / totalAllocated : 0,
    };
  }
  ...
  async resetTenantAdminPassword(
    ...
    const adminUser = tenant.users.find((u) =>
      u.roles.some((r) => r.name === Role.AdminDirector || r.name === Role.CEO),
    );
    if (!adminUser)
      throw new NotFoundException(
        "No AdminDirector or CEO user found for this tenant.",
      );
    ...
    await this.emailService
      ...
      .catch((err) =>
        this.logger.error(`Failed to send reset notification: ${err.message}`),
      );
