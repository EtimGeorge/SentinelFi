...
export class AuditService {
  ...
  ): Promise<void> { // Changed return type to void as it's fire-and-forget
    ...
    this.auditLogRepository.save(auditLog).catch((error: unknown) => {
      if (error instanceof Error) {
        this.logger.error(
          ...
          error.stack,
          JSON.stringify({ userId, action, tenantId, description, details, userEmail, ipAddress, userAgent }),
        );
      } else {
        this.logger.error(
          `Failed to save audit log event for action: ${action} by user: ${userEmail}`,
