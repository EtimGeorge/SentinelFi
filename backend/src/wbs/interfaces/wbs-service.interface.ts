...
export interface IWbsService {
  ...
  findAllWbsBudgets(
    options: GetWbsBudgetsDto,
  ): Promise<{ budgets: WbsBudgetEntity[]; total: number }>;
  findAllLiveExpenses(
    options: GetLiveExpensesDto,
  ): Promise<{ expenses: LiveExpenseEntity[]; total: number }>;
  findAllWbsBudgetsWithRollup(
    tenant_id: string,
    startDate?: string,
    endDate?: string,
  ...
  // Export Operations
  exportBudgetsToFormat(
    options: GetWbsBudgetsDto,
    format: "csv" | "pdf" | "xlsx" | "docx",
  ): Promise<Buffer>;
  exportExpensesToFormat(
    options: GetLiveExpensesDto,
    format: "csv" | "pdf" | "xlsx" | "docx",
  ): Promise<Buffer>;

  // Other Operations
  seedWbsDataForTenant(
    tenant_id: string,
    wbsData: any[],
    userId: string,
  ): Promise<WbsBudgetEntity[]>;
  findAllProjects(options: GetProjectsDto): Promise<{
    projects: (WbsBudgetEntity & { total_paid: number })[];
    total: number;
  }>; // This one still seems out of place, but exists in service
}
