...
*   `backend/src/database/tenancy-aware-data-source.ts` - the engine of multi-tenancy.
*   `backend/src/auth/jwt.strategy.ts` - the identity and caching gateway (Redis-backed).
*   `backend/src/common/interceptors/log-sanitization.interceptor.ts` - the automated PII masking layer.
