### STEP 102 - Update Global Navigation to Use Centralized Logo

**GUIDANCE:** We are replacing the hardcoded SVG logo placeholder in the `LayoutNav` component with the new `<Logo>` component. We will use the `sm` size for the navigation bar to maintain a compact, clean header.

**FILE PATH:** `./frontend/components/Layout/LayoutNav.tsx` (REPLACE entire file content)
...
NEXT ACTION: Save and overwrite the existing `./frontend/components/Layout/LayoutNav.tsx`.
