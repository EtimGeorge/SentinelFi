### STEP 101 - Update Login Page to Use Centralized Logo

**GUIDANCE:** We are replacing the hardcoded SVG logo placeholder in the `LoginPage` with the new, reusable `<Logo>` component. We will use the `lg` size for the main login page branding, as intended in the blueprint.

**FILE PATH:** `./frontend/pages/login.tsx` (REPLACE entire file content)
...
NEXT ACTION: Save and overwrite the existing `./frontend/pages/login.tsx`. We now update the navigation component.
